 ...
 */
uint16_t ATEMbase::getATEM_lastRemotePacketId() const	{
 ...
 * Get ATEM session ID
 */
uint16_t ATEMbase::getSessionID() const {
	return _sessionID;
}
 ...
 */
bool ATEMbase::isConnected() const	{
	return _isConnected;
}
 ...
 */
bool ATEMbase::hasInitialized() const	{
 ...
 */
bool ATEMbase::isRejected() const {
	return _isRejected;
}
...
void ATEMbase::_parsePacket(uint16_t packetLength)	{
      ...
      while (indexPointer < packetLength)  {
        ...
        char cmdStr[] = {
                (char)_packetBuffer[4], (char)_packetBuffer[5], (char)_packetBuffer[6], (char)_packetBuffer[7], '\0'};
...
uint8_t ATEMbase::getATEMmodel() const	{
	return _ATEMmodel;
}
