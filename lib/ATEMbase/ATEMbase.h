...
class ATEMbase
{
  ...
  public:
	...
	void begin(const IPAddress& ip, uint16_t localPort);
    void connect();
    void connect(boolean useFixedPortNumber);
    void runLoop();
	void runLoop(uint16_t delayTime);
		
	uint16_t getATEM_lastRemotePacketId() const;
	...
	static bool hasTimedOut(unsigned long time, unsigned long timeout);

	static float audioWord2Db(uint16_t input);
	static uint16_t audioDb2Word(float input);
	...
	uint8_t getATEMmodel() const;

  protected:
  	void _createCommandHeader(uint8_t headerCmd, uint16_t lengthOfData);
  	void _createCommandHeader(uint8_t headerCmd, uint16_t lengthOfData, uint16_t remotePacketID);
